...
# Rscript PCA.R > PCA_output.txt 2>&1
...
file_path <- "data_combined_full.csv"
...
pca_plot <- ggplot(pca_data, aes(x = PC1, y = PC2, color = Type, label = Sample)) +
  ...
  labs(title = "PCA Plot Isoform-Level removed BCH_1199",
...
# Save the plot
ggsave("PCA_isoform_level_removed_BCH1199.pdf", plot = pca_plot, width = 8, height = 6)